...
function extractTarGz(buffer, destDir) {
  const tmpFile = path.join(os.tmpdir(), `cq-${Date.now()}.tar.gz`);
...
function extractZip(buffer, destDir) {
  const tmpFile = path.join(os.tmpdir(), `cq-${Date.now()}.zip`);
  fs.writeFileSync(tmpFile, buffer);
  try {
...
async function main() {
  ...
  if (fs.existsSync(binaryPath)) {
    console.log(`cq binary already exists at ${binaryPath}`);
    return;
  }
  ...
  console.log(`Downloading cq v${VERSION} for ${getPlatformKey()}...`);
  ...
  try {
    ...
    console.log(`Installed cq to ${binaryPath}`);
  } finally {
    // Clean up temp dir
    try {
...
main().catch((e) => {
  console.error(`Unexpected error during cq installation: ${e.message}`);
  process.exit(1);
});
