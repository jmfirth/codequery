//! Stack graph rules for JavaScript.

use tree_sitter_stack_graphs::StackGraphLanguage;

use crate::error::ResolveError;

/// Vendored TSG source for JavaScript stack graph construction.
pub const TSG_SOURCE: &str = include_str!("../../tsg/javascript/stack-graphs.tsg");

/// Create a `StackGraphLanguage` for JavaScript.
///
/// Loads the vendored TSG rules and the JavaScript tree-sitter grammar.
///
/// # Errors
///
...
pub fn create_language() -> crate::error::Result<StackGraphLanguage> {
    ...
    StackGraphLanguage::from_str(grammar, TSG_SOURCE)
        .map_err(|e| ResolveError::RuleLoadError(format!("javascript: {e}")))
}
...
mod tests {
    ...
    fn test_create_language_succeeds() {
        ...
        assert!(
            result.is_ok(),
            "failed to create JavaScript language: {}",
