...
fn run(args: CqArgs) -> anyhow::Result<ExitCode> {
    ...
    match args.command {
        ...
        Command::Dead => commands::dead::run(
            args.project.as_deref(),
            args.scope.as_deref(),
            args.kind.as_deref(),
            args.limit,
        ...
        Command::Diagnostics { file } => commands::diagnostics::run(
            file.as_deref(),
            args.project.as_deref(),
            args.scope.as_deref(),
            mode,
        ...
        Command::Hover { location } => commands::hover::run(
            &location,
            args.project.as_deref(),
            mode,
            pretty,
        ...
        Command::Callchain { symbol } => commands::callchain::run(
            &symbol,
        ...
        Command::Hierarchy { symbol } => commands::hierarchy::run(
            &symbol,
            args.project.as_deref(),
            args.scope.as_deref(),
            lang_filter,
